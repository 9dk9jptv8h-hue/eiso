...
永想把每条记忆变成一串数字（向量），搜的时候比较这些数字的相似度。整个过程不调 API、不下模型，纯本地计算。
...
"小明喜欢Python，讨厌Java"
        ↓
  分词：小明明 / 喜欢Python / 讨厌Java / python / java
        ↓
  TF-IDF 计算每个词的重要程度
        ↓
  得到 256 个数字（向量），存进 SQLite
```
...
"推荐什么编程语言"
  ...
  跟数据库里每条记忆的向量算余弦相似度
        ↓
  "小明喜欢Python讨厌Java" 相似度 0.72 ← 最相关！
        ↓
  混合评分：语义相似度 × 0.55 + 重要性 × 0.30 + 新鲜度 × 0.15
        ↓
  返回最相关的 5 条
```
